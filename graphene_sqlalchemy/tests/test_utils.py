...
def test_get_session_custom_name():
    session = "My SQLAlchemy session"
    ...
    class Query(ObjectType):
        ...
        def resolve_x(self, info):
            return get_session(info.context, "custom_session_name")
    ...
    schema = Schema(query=Query)
    result = schema.execute(query, context_value={"custom_session_name": session})
    assert not result.errors
    assert result.data["x"] == session
